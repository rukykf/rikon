    ...
    export default {
        ...
        methods: {
            getData: async function(){
                ...
                let rolesData = await axios.get('http://localhost:3000/roles')
...
<template>
    <Layout>
            ...
            <div class="card mt-3">
                <div class="card-body">
                    ...
                    <div v-else-if="currentTab === 'Users'">
                        <div class="row">
                            <div class="col-12">
                                <button class="btn btn-primary mt-3 mb-5 px-5" @click.prevent.stop="showCreateUserModal">Add New User</button>
                            </div>
                        </div>
                        ...
                        <div v-show="true" class="table-responsive  table-hover mb-0">
                            ...
                            >
                                ...
                                <template v-slot:cell(actions)="row">
                                    <a class="badge badge-primary text-white mx-1" href="#" @click.stop.prevent="showEditUserModal(row.item)">Edit User / Reset Password</a>
                                    <a class="badge badge-danger text-white mx-1" href="#" @click.stop.prevent="deleteUser(row.item, $event.target)">Delete User</a>
                                </template>
                           ...
                           <table class="table table-hover">
                               ...
                               <tbody>
                                    <tr v-for="(role, index) in roles" :key="index">
                                        <td>{{index + 1}}</td>
                                        <td>{{role.name}}</td>
                                        <td><span v-for="(permission, roleIndex) in role.permissions" :key="roleIndex">{{permission}}, </span></td>
                                        <td> <a class="badge badge-primary text-white mx-1" href="#" @click.stop.prevent="showEditRoleModal(role)">Edit Role</a></td>
                                    </tr>
                               </tbody>
                           </table>
                        </div>
        ...
        <div>
            <b-modal id="save-user" size="xl" hide-footer header-bg-variant="dark" title="Save User">
                <SaveUser :user="selectedUser"></SaveUser>
            </b-modal>
            <b-modal id="save-role" size="xl" hide-footer header-bg-variant="dark" title="Save Role">
                <SaveUserRole :role="selectedRole"></SaveUserRole>
            </b-modal>
            <b-modal id="create-role" size="xl" hide-footer header-bg-variant="dark" title="Save Role">
                <SaveUserRole addMultiple></SaveUserRole>
            </b-modal>
            <b-modal id="create-user" size="xl" hide-footer header-bg-variant="dark" title="Save User">
