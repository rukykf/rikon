<script>
export default {
 name: "display-customer-order-details",
 props: {
  details: {
   type: Object,
   required: true,
...
<template>
 <div>
  ...
  <b-row class="mb-2">
   ...
   <b-col
    >{{ details.updated_at | humanDate }} <span class="font-italic">{{ details.updated_at | humanTime }}</span>
   </b-col>
  </b-row>
   ...
   <b-col>
    <b-row>
     <b-col cols="12" v-for="item in details.order_items"
      >x{{ item.quantity }} {{ item.unit }}(s) of {{ item.name }} at {{ item.price_per_unit | money }} per unit with
      total of {{ item.amount | money }}
     </b-col>
    </b-row>
   </b-col>
  </b-row>
